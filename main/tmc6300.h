 ...
 * inputs (UL, VL, WL) are held permanently HIGH so the complementary
 * low-side FETs conduct whenever the corresponding high-side FET is
 * off, completing the current path through each motor phase:
 *
 *   UH (PWM)  -> Phase U high-side gate
 *   VH (PWM)  -> Phase V high-side gate
 *   WH (PWM)  -> Phase W high-side gate
 *   UL (HIGH) -> Phase U low-side gate (always on)
 *   VL (HIGH) -> Phase V low-side gate (always on)
 *   WL (HIGH) -> Phase W low-side gate (always on)
 *
 * The VIO pin is tied permanently to +3.3 V so the driver is always
 * active (no software standby control).
 */
...
/**
 * Initialise one TMC6300 (three LEDC PWM channels + low-side enables).
 *
 ...
 * @param ul_gpio      UL GPIO (driven HIGH -> phase U low-side enable).
 * @param vl_gpio      VL GPIO (driven HIGH -> phase V low-side enable).
 * @param wl_gpio      WL GPIO (driven HIGH -> phase W low-side enable).
